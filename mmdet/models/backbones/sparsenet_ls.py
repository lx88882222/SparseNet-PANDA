    ...
    def forward(self, x, hw_shape, keep_token_indices):
        ...
        query_windows_sparse = torch.gather(query_windows, sparse_grad=True, dim=1,
                                            index=keep_token_indices.view(B, K, 1, 1, 1).repeat(1, 1,
                                                                                                ...
                                                                                                self.window_size,
                                                                                                C))
        # print("gather:", time.time()-haha_point)
        ...
        x = query_windows.scatter(src=output, dim=1,
                                             index=keep_token_indices.view(B, K, 1, 1, 1).repeat(1, 1,
                                                                                                 ...
                                                                                                 self.window_size,
                                                                                                 C))
        # print("scatter:", time.time()-haha_point)
...
class BlockSequence(BaseModule):
    ...
    def score_generator(self, x, hw_shape):
        ...
        # import cv2
        # import numpy as np
        # img = '/home/liwenxi/panda/patch_mix/IMG_12_10_0296.jpg'
        # img = cv2.imread(img)
        # h, w, c = img.shape
        # temp = x_score.squeeze().view(H, W, C)[:-2, :-2, 0].cpu().numpy()
        # temp = temp / (np.max(temp) -np.min(temp))
        # alpha = np.clip(temp*255, a_min=0, a_max=255)
        # alpha = cv2.resize(alpha, dsize=(w, h))[:, :, np.newaxis]
        # alpha = np.array(alpha, dtype=np.uint8)
        # img = np.concatenate([img, alpha], axis=2)
        # plt.imsave(str(uuid.uuid4())+".png", img)

        # print(torch.topk(x_score.squeeze().view(H, W, C)[:, :, 0], 3))
        # haha = x_score.squeeze().view(H, W, C)
        # haha[-(self.window_size-pad_b): , :, :]= torch.min(haha)
        # haha[: , -(self.window_size-pad_r):, :]= torch.min(haha)
        # plt.imsave(str(uuid.uuid4())+".png", haha[:, :, 0].cpu().numpy(), cmap=CM.jet)
        ...
        # # print(x_score.shape)
        # # print(x_score.view(H, W, C)[:, :, 0])
        # x_score_b = x_score.squeeze().view(H, W, C)
        # x_score_b[-(self.window_size-pad_b):, -(self.window_size-pad_r):, :] = 0
        # all_num = x_score_b.squeeze().view(H, W, C)[:, :, 0].cpu().numpy().reshape(-1)
        #
        # all_num = np.sort(all_num)
        # t = all_num[int(W*H*0.8)]
        # # print(all_num)
        # # top_k = torch.topk(x_score.squeeze().view(H, W, C)[:, :, 0].reshape(-1), int(H*W*0.4))[0]
        # # top_k = torch.min((top_k))
        #
        # x_score_copy = x_score_b
        # x_score_copy[x_score_copy <= t] = 0
        # x_score_copy[x_score_copy > t] = 1
        #
        # plt.imsave(str(uuid.uuid4())+".png", x_score_copy.squeeze().view(H, W, C)[:-2, :-2, 0].cpu().numpy(), cmap=CM.gray)



        # transformer for global level
        # query_global = x_pooled.flatten(2).transpose(1, 2)
        query_global = x_pooled

        for block in self.global_blocks:
            att_global = block(query_global)
        # att_global = self.score_norm(att_global)
        x_att_global_pooled = att_global.view(B, -1, H_pad // self.window_size, W_pad // self.window_size)
        # query_global = query_global.transpose(1, 2).view(B, -1, H_pad // self.window_size,
        #                                                       W_pad // self.window_size)
        # B, H, W, C
        x_att_global = F.interpolate(x_att_global_pooled, scale_factor=self.window_size).permute(0, 2, 3, 1)
        # x_global = F.interpolate(query_global, scale_factor=self.window_size).permute(0, 2, 3, 1)
        if pad_r > 0 or pad_b:
            x_att_global = x_att_global[:, :H, :W, :].contiguous().view(B, -1, C)
            # x_global = x_global[:, :H, :W, :].contiguous().view(B, -1, C)
        else:
            x_att_global = x_att_global.contiguous().view(B, -1, C)
            # x_global = x_global.contiguous().view(B, -1, C)

        return keep_token_indices, x_score, x_att_global
